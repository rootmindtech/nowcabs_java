...
public class FCMNotification extends Helper implements Runnable {
			...
			if (dataArrayWrapper.recordFound == true && dataArrayWrapper.notificationWrapper != null
				...
				for (int i = 0; i < dataArrayWrapper.notificationWrapper.length; i++) {
					...
					if (notificationWrapper.recordFound == true) {
						...
						if (riderWrapper != null && !Utility.isEmpty(riderWrapper.fcmToken)
								&& !Utility.isEmpty(notificationWrapper.notificationCode)) {
							...
							case Constants.REG_SERVICE_PN:
								MSG_CODE = "SERVICE";
								message = "You have updated Service";
								break;
...
// classLoader.getResourceAsStream("nowcabs_icon48x48.png");
...
//
//
// public FCMNotification(String name, String refNo, String ID) {
//
// threadName = name;
// this.refNo = refNo;
// this.ID = ID;
...
// thread.start();
// System.out.println("Started New thread " + threadName);
//
// // }
//
// }
...
// studentDiaryWrapper.academicYearID =
...
//
// sql = "SELECT a.StudentID, a.StudentName as StudentName, a.Surname as
// Surname, b.DeviceToken as DeviceToken FROM StudentProfile a, Users b WHERE
// a.StudentId=b.StudentId and a.Status<>'INACTIVE' and b.UserGroup='STUDENT' "
// + " and a.AcademicYearID=? and a.RefNo=? and a.StudentID=? and b.StudentID=?
// and a.SchoolID=?";
//
...
// // +studentDiaryWrapper.studentID );
//
// message = birthdayMessage.replace("{{studentName}} {{surname}}",
// studentName);
//
...
// !vector.contains(deviceToken.trim())) {
...
// result = sendMessage(null, registration_ids, message, "Birthday Message",
// "myicon",
// studentDiaryWrapper.schoolID);
//
// }
...
// "UPDATE StudentNotification SET Message=?, Delivered=?, MessageDateTime=?,
// GCMResponse=? "
// + " WHERE Delivered <>'Y' and MessageID=? and RefNo=? and StudentID=? and
// SchoolID=?");
//
...
// public String sendMessage(String toTokenId, String[] registration_ids, String
// body, String title, String icon) throws Exception {
//
// Connection con = null;
// ResultSet resultSet = null;
//
...
//
// sql = "SELECT FCMKey,FCMActivate,FCMURL from Parameter";
//
// pstmt = con.prepareStatement(sql);
//
// resultSet = pstmt.executeQuery();
// if (resultSet.next()) {
//
// fcmKey = Utility.trim(resultSet.getString("FCMKey"));
// fcmActivate = Utility.trim(resultSet.getString("FCMActivate"));
// fcmURL = Utility.trim(resultSet.getString("FCMURL"));
// }
//
// resultSet.close();
// pstmt.close();
//
// // ----------
// if (fcmActivate != null && fcmActivate.equals("Y")) {
...
//
// System.out.println("FCM toTokenId " + toTokenId);
//
//
...
// JsonArray registration_ids_array =
...
// String request = fcmURL; // "https://gcm-http.googleapis.com/gcm/send";
// URL url = new URL(request);
// HttpURLConnection conn = (HttpURLConnection) url.openConnection();
// conn.setDoOutput(true);
// conn.setInstanceFollowRedirects(false);
// conn.setRequestProperty("Authorization", "key=" + fcmKey); //
// AIzaSyChya7IJ7KWKIlJbQIxv-apjxtcuStIBUg
...
// }
//
// // -----Login failure alerts to users from App ---
// public void sendLoginFailureMessage(String userid, String toTokenId) throws
// Exception {
//
// try {
//
...
//
// String registrationToken = toTokenId.trim();
//
// System.out.println("FCMTokenId " + registrationToken);
//
// // See documentation on defining a message payload.
// // Message message = Message.builder()
// // .putData("Maabadi (myschool) app login unsuccessful for userid " + userid
// +
// // ". If your school is not registered yet, please register using Register
// // School option")
// // .setToken(registrationToken)
// // .build();
//
// Message message =
// Message.builder().setAndroidConfig(AndroidConfig.builder().setTtl(3600 *
// 1000) // 1 hour
// // in
// // milliseconds
// .setPriority(AndroidConfig.Priority.NORMAL)
// .setNotification(AndroidNotification.builder().setTitle("maabadi (my
// school)")
// .setBody("maabadi (myschool) app login unsuccessful for userid " + userid
// + ". If your school is not registered yet, please register using Register
// School option")
// .build())
// .build())
...
// .build();
//
// //.setIcon(maabadiIcon).setColor("#f45342").
//
//
// // See documentation on defining a message payload.
//// Message message = Message.builder()
...
// // ----Grade all code--
//
// pstmt = con.prepareStatement("SELECT GradeAllCode from MST_Parameter where
// SchoolID=?");
//
...
// // --------
//
// pstmt = con.prepareStatement("SELECT MessageID, Message, MessageDateTime,
// Delivered, GradeList, MakerID, "
// + " MakerDateTime, SchoolID FROM SchoolMessage WHERE Delivered <>'Y' and
// SchoolID=?");
//
...
////
//// schoolMessageWrapper.messageID =
// Utility.trim(resultSet.getString("MessageID"));
//// schoolMessageWrapper.message =
// Utility.trim(resultSet.getString("Message"));
//// schoolMessageWrapper.messageDateTime =
// Utility.trim(resultSet.getString("MessageDateTime"));
//// schoolMessageWrapper.delivered =
// Utility.trim(resultSet.getString("Delivered"));
//// schoolMessageWrapper.gradeList =
// Utility.trim(resultSet.getString("GradeList"));
//// schoolMessageWrapper.makerID =
// Utility.trim(resultSet.getString("MakerID"));
//// schoolMessageWrapper.makerDateTime =
// Utility.setDate(resultSet.getString("MakerDateTime"));
//// schoolMessageWrapper.schoolID =
// Utility.setDate(resultSet.getString("SchoolID"));
////
//// schoolMessageWrapper.recordFound = true;
////
//// if
// (schoolMessageWrapper.gradeList.toUpperCase().trim().equals(gradeAllCode)) {
//// sql = "SELECT a.StudentID, b.DeviceToken as DeviceToken FROM StudentProfile
// a, Users b WHERE a.StudentId=b.StudentId and a.Status<>'INACTIVE' and
// b.UserGroup='STUDENT' and a.SchoolID=?";
//// } else {
////
//// sql = "SELECT a.StudentID, b.DeviceToken as DeviceToken FROM StudentProfile
// a, Users b WHERE a.StudentId=b.StudentId and a.Status<>'INACTIVE' and
// b.UserGroup='STUDENT' and a.GradeId=? and a.SchoolID=?";
//// }
////
//// pstmtSub = con.prepareStatement(sql);
////
...
//// // single device id
//// String toTokenId = vector.get(0);
//// result = sendMessage(toTokenId, null, schoolMessageWrapper.message, "School
// Message", "myicon",
//// schoolMessageWrapper.schoolID);
//// } else {
//// // multiple devices at the same time
//// String[] registration_ids = new String[vector.size()];
//// vector.copyInto(registration_ids);
//// result = sendMessage(null, registration_ids, schoolMessageWrapper.message,
// "School Message",
...
////
//// pstmtSub = con
//// .prepareStatement("UPDATE SchoolMessage SET Delivered=?,
// MessageDateTime=?,GCMResponse=? "
...
// "SELECT AcademicYearID,RefNo,StudentID,MessageID, Message, MessageDateTime,
// Delivered, GradeID,SectionID, MakerID, "
...
// Utility.trim(resultSet.getString("MessageID"));
//// studentDiaryWrapper.message = Utility.trim(resultSet.getString("Message"));
//// studentDiaryWrapper.messageDateTime =
// Utility.trim(resultSet.getString("MessageDateTime"));
//// studentDiaryWrapper.delivered =
// Utility.trim(resultSet.getString("Delivered"));
//// studentDiaryWrapper.gradeID = Utility.trim(resultSet.getString("GradeID"));
...
//// studentDiaryWrapper.makerDateTime =
// Utility.setDate(resultSet.getString("MakerDateTime"));
//// studentDiaryWrapper.schoolID =
// Utility.setDate(resultSet.getString("SchoolID"));
////
//// studentDiaryWrapper.recordFound = true;
////
//// sql = "SELECT a.StudentID , b.DeviceToken as DeviceToken FROM
// StudentProfile a, Users b WHERE a.Status<>'INACTIVE' and
// b.UserGroup='STUDENT'"
//// + " and a.AcademicYearID=? and a.GradeId=? and a.sectionID=? and
// a.SchoolID=?";
////
//// System.out.println("StudentID in send diary message " +
// studentDiaryWrapper.studentID);
//// System.out.println("RefNo in send diary message " +
// studentDiaryWrapper.refNo);
////
//// if (studentDiaryWrapper.studentID != null &&
// !studentDiaryWrapper.studentID.trim().equals("")) {
...
//// if (studentDiaryWrapper.studentID != null &&
// !studentDiaryWrapper.studentID.trim().equals("")) {
//// System.out.println("if StudentID in send diary message " +
// studentDiaryWrapper.studentID);
//// System.out.println("if RefNo in send diary message " +
// studentDiaryWrapper.refNo);
...
//// // single device id
//// String toTokenId = vector.get(0);
//// result = sendMessage(toTokenId, null, studentDiaryWrapper.message, "School
// Diary", "myicon",
//// studentDiaryWrapper.schoolID);
//// } else {
//// // multiple devices at the same time
//// String[] registration_ids = new String[vector.size()];
//// vector.copyInto(registration_ids);
//// result = sendMessage(null, registration_ids, studentDiaryWrapper.message,
// "School Diary",
...
////
//// pstmtSub = con
//// .prepareStatement("UPDATE StudentDiary SET Delivered=?,
// MessageDateTime=?,GCMResponse=? "
//// + " WHERE Delivered <>'Y' and MessageID=? and SchoolID=?");
//// pstmtSub.setString(1, "Y");
//// pstmtSub.setTimestamp(2, Utility.getCurrentTime());
...
// public AbstractWrapper sendAttendanceMessage(String schoolID) throws
// Exception {
//
// Connection con = null;
// ResultSet resultSet = null;
...
// //ParameterWrapper parameterWrapper = (ParameterWrapper)
// popoverHelper.fetchParameters(schoolID);
// //absentMessage = parameterWrapper.absentMessage;
//
// // ------------
//
// pstmt = con.prepareStatement(
// "SELECT AcademicYearID,RefNo,StudentID,CalendarDate,MessageID, Message,
// Delivered, GradeID,SectionID, MorningStatus, "
// + " EveningStatus, SchoolID FROM StudentAttenMessage WHERE Delivered <>'Y'
// and SchoolID=?");
//
// pstmt.setString(1, Utility.trim(schoolID));
//
...
//// studentAttendanceWrapper.academicYearID =
// Utility.trim(resultSet.getString("AcademicYearID"));
//// studentAttendanceWrapper.refNo =
// Utility.trim(resultSet.getString("RefNo"));
//// studentAttendanceWrapper.studentID =
// Utility.trim(resultSet.getString("StudentID"));
//// studentAttendanceWrapper.calendarDate =
// Utility.setDate(resultSet.getString("CalendarDate"));
//// studentAttendanceWrapper.messageID =
// Utility.trim(resultSet.getString("MessageID"));
//// studentAttendanceWrapper.message =
// Utility.trim(resultSet.getString("Message"));
//// studentAttendanceWrapper.delivered =
// Utility.trim(resultSet.getString("Delivered"));
//// studentAttendanceWrapper.gradeID =
// Utility.trim(resultSet.getString("GradeID"));
//// studentAttendanceWrapper.sectionID =
// Utility.trim(resultSet.getString("SectionID"));
//// studentAttendanceWrapper.morningStatus =
// Utility.trim(resultSet.getString("MorningStatus"));
//// studentAttendanceWrapper.eveningStatus =
// Utility.trim(resultSet.getString("EveningStatus"));
//// studentAttendanceWrapper.schoolID =
// Utility.trim(resultSet.getString("SchoolID"));
////
...
// Surname, b.DeviceToken as DeviceToken FROM StudentProfile a, Users b WHERE
// a.StudentId=b.StudentId and a.Status<>'INACTIVE' and b.UserGroup='STUDENT' "
//// + " and a.AcademicYearID=? and a.RefNo=? and a.StudentID=? and
// b.StudentID=? and a.SchoolID=?";
////
//// pstmtSub = con.prepareStatement(sql);
////
//// pstmtSub.setString(1,
// Utility.trim(studentAttendanceWrapper.academicYearID));
...
////
//// message = message.replace("{{calendarDate}}",
// studentAttendanceWrapper.calendarDate);
////
...
//// // single device id
//// String toTokenId = vector.get(0);
//// result = sendMessage(toTokenId, null, message, "Student Attendance",
// "myicon",
//// studentAttendanceWrapper.schoolID);
//// } else {
//// // multiple devices at the same time
//// String[] registration_ids = new String[vector.size()];
//// vector.copyInto(registration_ids);
//// result = sendMessage(null, registration_ids, message, "Student Attendance",
// "myicon",
//// studentAttendanceWrapper.schoolID);
////
//// }
////
//// pstmtSub = con.prepareStatement(
//// "UPDATE StudentAttenMessage SET Message=?, Delivered=?, MessageDateTime=?,
// GCMResponse=? "
//// + " WHERE Delivered <>'Y' and MessageID=? and RefNo=? and StudentID=? and
// SchoolID=?");
////
